import { GLTFAnimation, GLTF, ValidationMessage, Severity, AnimationPath, AnimationInterpolation } from '../types';

export class AnimationValidator {
  validate(animation: GLTFAnimation, index: number, gltf: GLTF): ValidationMessage[] {
    const messages: ValidationMessage[] = [];

    // Check for unexpected properties first to match expected order
    const expectedProperties = ['channels', 'samplers', 'name', 'extensions', 'extras'];
  ...
  private validateChannel(channel: any, animationIndex: number, channelIndex: number, gltf: GLTF): ValidationMessage[] {
    const messages: ValidationMessage[] = [];
  ...
  private validateTarget(target: any, animationIndex: number, channelIndex: number, gltf: GLTF, channel?: any): ValidationMessage[] {
    const messages: ValidationMessage[] = [];
    ...
    if (!target.path) {
      ...
      const hasAnimationPointerExt = (target.extensions && target.extensions['KHR_animation_pointer']) ||
                                    (channel && channel['extensions'] && channel['extensions']['KHR_animation_pointer']);
  ...
  private validateSampler(sampler: any, animationIndex: number, samplerIndex: number, gltf: GLTF): ValidationMessage[] {
    const messages: ValidationMessage[] = [];
    ...
    if (sampler.input === undefined) {
      ...
      if (inputAccessor && (inputAccessor.min === undefined || inputAccessor.max === undefined)) {
        messages.push({
    ...
    if (sampler.interpolation !== undefined) {
      ...
      } else if (!Object.values(AnimationInterpolation).includes(sampler.interpolation as any)) {
        messages.push({
          code: 'INVALID_VALUE',
  ...
  private checkDuplicateTargets(channels: any[], animationIndex: number, messages: ValidationMessage[]): void {
  ...
  private getExpectedOutputFormat(gltf: GLTF, animationIndex: number, samplerIndex: number): { type: string; componentType: number; path: string } | null {
