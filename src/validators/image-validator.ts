...
export class ImageValidator {
  ...
  validate(image: GLTFImage, index: number, gltf: GLTF): ValidationMessage[] {
    ...
    const expectedProperties = ['uri', 'mimeType', 'bufferView', 'name', 'extensions', 'extras'];
    ...
    if (image['extensions']) {
      for (const extensionName in image['extensions']) {
  ...
  async validateExternalResources(image: GLTFImage, index: number): Promise<ValidationMessage[]> {
    const messages: ValidationMessage[] = [];
  ...
  private validateImageData(data: Uint8Array, format: string): { code: string; message: string } | null {
    // Validate image data integrity based on format
    switch (format) {
      case 'image/jpeg':
        const jpegError = this.validateJPEGData(data);
        return jpegError ? { code: 'IMAGE_DATA_INVALID', message: `Image data is invalid. ${jpegError}` } : null;
      case 'image/png':
        ...
        if (pngError) {
          ...
          if (pngError.includes('Unexpected end of image stream')) {
            ...
            return { code: 'IMAGE_DATA_INVALID', message: `Image data is invalid. ${pngError}` };
          }
        }
        return null;
  ...
  private getImageDimensions(data: Uint8Array, format: string): { width: number; height: number } | null {
    switch (format) {
      case 'image/png':
  ...
  private getPNGDimensions(data: Uint8Array): { width: number; height: number } | null {
  ...
  private getJPEGDimensions(data: Uint8Array): { width: number; height: number } | null {
