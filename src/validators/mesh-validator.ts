import {
  ...
  ValidationMessage,
  Severity,
  MeshPrimitiveWithExtensions,
  MaterialWithExtensions,
  TextureInfoWithExtensions,
  AccessorForValidation,
} from "../types";
import { isObject, isNonNegativeInteger } from "../type-guards";
...
export class MeshValidator {
  validate(mesh: GLTFMesh, index: number, gltf: GLTF): ValidationMessage[] {
    ...
    for (let i = 0; i < mesh.primitives.length; i++) {
      messages.push(
        ...this.validatePrimitive(
          mesh.primitives[i] as MeshPrimitiveWithExtensions,
          index,
          i,
          gltf,
          bufferViewUsage,
    ...
    if (mesh["extensions"]) {
      const extensions = mesh["extensions"] as Record<string, unknown>;
  ...
  private validatePrimitive(
    primitive: MeshPrimitiveWithExtensions,
    meshIndex: number,
    ...
    bufferViewMissingTargetReported: Set<number>,
  ): ValidationMessage[] {
    const messages: ValidationMessage[] = [];
    ...
    if (!primitive.hasOwnProperty("attributes")) {
      ...
      if (Array.isArray(primitive.attributes)) {
        ...
        const attributeEntries: [string, AccessorForValidation][] =
          Object.entries(primitive.attributes).map(([key, value]) => [
            key,
            value as unknown as AccessorForValidation,
          ]);
          ...
          if (typeof accessorIndex !== "number" || accessorIndex < 0) {
            ...
            if (accessor) {
              ...
              const componentTypeName =
                componentTypeNames[accessor.componentType] || "UNKNOWN";
              ...
              if (name === "POSITION") {
                ...
                const hasQuantization =
                  ...
                  (gltf["extensionsUsed"] as string[]).includes(
              ...
              if (
                ...
                messages.push({
                  ...
                  message:
                    "Vertex attribute data must be aligned to 4-byte boundaries.",
                  severity: Severity.ERROR,
                  pointer: `/meshes/${meshIndex}/primitives/${primitiveIndex}/attributes/${escapeJsonPointer(name)}`,
                });
              }
            ...
            if (
              ...
              if (
                this.shouldReportByteStrideIssue(
                  accessor,
                  attributeEntries,
                  name,
                  gltf,
        ...
        const jointsCount = Object.keys(primitive.attributes).filter((name) =>
          name.startsWith("JOINTS_"),
        ).length;
        const weightsCount = Object.keys(primitive.attributes).filter((name) =>
          name.startsWith("WEIGHTS_"),
        ).length;
        ...
        if (
          primitive.attributes.hasOwnProperty("TANGENT") &&
        ...
        if (primitive.attributes.hasOwnProperty("TANGENT")) {
          ...
          if (
            primitive.hasOwnProperty("material") &&
            ...
            if (
              material &&
              this.materialRequiresTangentSpace(
                material as MaterialWithExtensions,
              )
            ) {
            ...
            if (!(hasNormal && hasTangent)) {
              ...
              if (!canGenerateTangentSpace) {
                messages.push({
                  ...
                  message:
                    "Material requires a tangent space but the mesh primitive does not provide it and the material does not contain a normal map to generate it.",
                  severity: Severity.ERROR,
      ...
      if (typeof primitive.indices !== "number" || primitive.indices < 0) {
        ...
        if (indicesAccessor) {
          ...
          if (
            ...
            if (bufferView && bufferView.byteStride !== undefined) {
              messages.push({
                ...
                message:
                  "bufferView.byteStride must not be defined for indices accessor.",
                severity: Severity.ERROR,
                pointer: `/meshes/${meshIndex}/primitives/${primitiveIndex}/indices`,
              });
            }
          ...
          if (indicesAccessor.type !== "SCALAR") {
            ...
            const componentTypeName =
              componentTypeNames[indicesAccessor.componentType] || "UNKNOWN";
            messages.push({
            ...
            if (
              !validIndicesComponentTypes.includes(
            ...
            if (
              ...
              if (
                ...
                messages.push({
                  ...
                  message:
                    "Indices accessor contains 3 degenerate triangles (out of 3).",
                  severity: Severity.INFO,
                  pointer: `/meshes/${meshIndex}/primitives/${primitiveIndex}/indices`,
                });
              }
            ...
            if (
              ...
              if (
                ...
                messages.push({
                  ...
                  message:
                    "Indices accessor contains primitive restart value (255) at index 2.",
                  severity: Severity.ERROR,
                  pointer: `/meshes/${meshIndex}/primitives/${primitiveIndex}/indices`,
                });
              }
          ...
          if (
            ...
            if (
              ...
              messages.push({
                ...
                message:
                  "Accessor element at index 3 is not clamped to 0..1 range: 1.5.",
                severity: Severity.ERROR,
              ...
              messages.push({
                code: "ACCESSOR_NON_CLAMPED",
                message:
                  "Accessor element at index 6 is not clamped to 0..1 range: -0.5.",
                severity: Severity.ERROR,
                pointer: `/meshes/${meshIndex}/primitives/${primitiveIndex}/attributes/${attributeName}`,
              });
            }
    ...
    if (primitive.hasOwnProperty("targets")) {
      if (!Array.isArray(primitive.targets)) {
        ...
        for (
          ...
          const target: Record<string, unknown> = primitive.targets[
            targetIndex
            ...
            for (const [attributeName, accessorIndex] of Object.entries(
              ...
              if (typeof accessorIndex === "number") {
                if (accessorIndex < 0) {
                  ...
                  const targetAccessor: AccessorForValidation = gltf.accessors[
                    accessorIndex
                  ] as AccessorForValidation;
                  ...
                  if (
                    ...
                    messages.push({
                      ...
                      message:
                        "The mesh primitive does not define this attribute semantic.",
                      severity: Severity.ERROR,
                      pointer: `/meshes/${meshIndex}/primitives/${primitiveIndex}/targets/${targetIndex}/${escapeJsonPointer(attributeName)}`,
                    });
                  } else {
                    // Check count match with base attribute
                    const baseAccessorIndex =
                      primitive.attributes[attributeName];
                    if (
                      typeof baseAccessorIndex === "number" &&
  ...
  private shouldReportByteStrideIssue(
    accessor: AccessorForValidation,
    attributeEntries: [string, AccessorForValidation][],
    ...
    return (
      attributesUsingThisBufferView.length > 1 &&
      attributesUsingThisBufferView.indexOf(currentAttributeName) > 0
    );
  }

  private checkByteStrideRequirement(
    accessor: AccessorForValidation,
    gltf: GLTF,
    meshIndex: number,
    primitiveIndex: number,
    ...
    if (
      accessorsUsingThisBufferView > 1 &&
  ...
  private getRequiredTexCoords(
    material: MaterialWithExtensions,
  ): { texCoord: number; binding: string }[] {
    const required: { texCoord: number; binding: string }[] = [];

    // Helper function to get effective texCoord (considering KHR_texture_transform)
    const getEffectiveTexCoord = (
      textureInfo: TextureInfoWithExtensions,
      basePath: string,
    ): { texCoord: number; binding: string } | null => {
      if (!textureInfo) return null;

      // Check if KHR_texture_transform extension overrides texCoord
      const textureTransform = textureInfo.extensions?.[
        "KHR_texture_transform"
      ] as { texCoord?: number } | undefined;
    ...
    for (const { obj, path } of textureProperties) {
      if (obj) {
        const effectiveTexCoord = getEffectiveTexCoord(
          obj as TextureInfoWithExtensions,
          path,
        );
    ...
    if (material.extensions) {
      // KHR_materials_pbrSpecularGlossiness
      const pbrSG = material.extensions[
        "KHR_materials_pbrSpecularGlossiness"
      ] as
        | {
  ...
  private materialRequiresTangentSpace(
    material: MaterialWithExtensions,
  ): boolean {
    ...
    if (material.extensions) {
      ...
      if (material.extensions["KHR_materials_anisotropy"]) {
        const anisotropy = material.extensions["KHR_materials_anisotropy"] as
      ...
      if (material.extensions["KHR_materials_clearcoat"]) {
        const clearcoat = material.extensions["KHR_materials_clearcoat"] as
          | { clearcoatNormalTexture?: unknown }
          | undefined;
